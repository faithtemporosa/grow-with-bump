...
const Header = () => {
  ...
  return <>
      <MiniCartPanel />
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isVisible ? 'translate-y-0' : '-translate-y-full'} ${isScrolled ? "glass border-b border-primary/10 shadow-neon backdrop-blur-xl" : "bg-gradient-to-b from-background/80 to-transparent backdrop-blur-sm"}`}>
        <div className="container mx-auto px-3 sm:px-4 py-3 sm:py-4">
        <div className="flex items-center justify-between">
          <Link to="/" onClick={() => window.scrollTo(0, 0)} className="flex items-center gap-2 sm:gap-3 group">
            <img src={logo} alt="Bump AI Logo" className={`w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 object-contain cursor-pointer transition-all duration-300 group-hover:scale-110 ${isScrolled ? 'drop-shadow-glow' : 'drop-shadow-lg'}`} style={{
              mixBlendMode: 'normal',
              isolation: 'isolate',
            ...
            <span className="text-lg sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent group-hover:scale-105 transition-transform duration-300 flex items-center gap-1">
              <img src={robotMascot} alt="Robot Mascot" />
              Bump AI
            </span>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-3 xl:gap-4">
            ...
            {user ? <>
                {isAdmin && <Link to="/admin" onClick={() => window.scrollTo(0, 0)}>
        ...
        {isMobileMenuOpen && <nav className="lg:hidden mt-4 pb-4 flex flex-col gap-4 animate-fade-in border-t border-border/20 pt-4">
            <Link to="/catalog" className="text-foreground hover:text-primary transition-smooth text-left" onClick={() => {
            setIsMobileMenuOpen(false);
            window.scrollTo(0, 0);
          }}>
              Automations
            ...
            <Link to="/cart" className="text-foreground hover:text-primary transition-smooth text-left flex items-center gap-2" onClick={() => {
            setIsMobileMenuOpen(false);
            window.scrollTo(0, 0);
          }}>
            ...
            <Link to="/my-orders" className="text-foreground hover:text-primary transition-smooth text-left flex items-center gap-2" onClick={() => {
            setIsMobileMenuOpen(false);
            window.scrollTo(0, 0);
          }}>
